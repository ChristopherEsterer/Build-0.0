...
float SpineApp::MaptoX(std::deque<double>& dataBuf, float& MAX, float& MIN, double& data)
{
...
float SpineApp::MaptoY(std::deque<double>& dataBuf, float& MAX, float& MIN, double& data)
{
	double DataMax = *std::max_element(dataBuf.begin(), dataBuf.end());
...
void SpineApp::SpineWarning(int i) {
// int i case for sevarity of warning. 0 Nothing
	switch (i) {
	case 0:
		break;
	case 1:
...
DWORD SpineApp::Run(HINSTANCE hInstance, int nCmdShow, HWND hDlg, int wmId)
{
	...
	NULL,
	(DLGPROC)SpineApp::MessageRouter,
...
void SpineApp::setJointTypes(int J0, int J1)
{
	JointType0 = J0;
	JointType1 = J1;
}
void SpineApp::pushData(double data, double time)
{
	if (dataBuffer[0].size() < cDataBufferSize)
	{
		if (dataBuffer[1].size() == 0)
...
void SpineApp::setOptiBodyClass(void* UBC)
{
	m_UserBody = UBC;
}
LRESULT CALLBACK SpineApp::MessageRouter(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	SpineApp* pThis = NULL;

	if (WM_INITDIALOG == uMsg)
	{
		pThis = reinterpret_cast<SpineApp*>(lParam);
		SetWindowLongPtr(hWnd, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(pThis));
	}
	else
	{
		pThis = reinterpret_cast<SpineApp*>(::GetWindowLongPtr(hWnd, GWLP_USERDATA));
	}
...
LRESULT SpineApp::DlgProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
...
void SpineApp::Display(void)
{
	if (m_hWnd)
	{
		...
		if (SUCCEEDED(hr) && m_pRenderTarget)
		{
			...
			float width = rct.right;
			float height = rct.bottom-58;
			float zero = 0;
...
bool SpineApp::SetStatusMessage(_In_z_ WCHAR* szMessage, DWORD nShowTimeMsec, bool bForce)
{
...
HRESULT SpineApp::EnsureDirect2DResources()
{
	HRESULT hr = S_OK;
...
/// </summary>
void SpineApp::DiscardDirect2DResources()
{
